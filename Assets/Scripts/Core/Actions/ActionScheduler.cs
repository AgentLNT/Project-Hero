...
namespace ProjectHero.Core.Actions
{
    ...
    public static class ActionScheduler
    {
        public static void ScheduleAttack(BattleTimeline timeline, CombatUnit attacker, Action action, float startTime, GridDirection? targetDirection = null)
        {
            ...
            timeline.Schedule(startTime, windupIntent, $"{attacker.name} starts {action.Name}");
            ...
            timeline.Schedule(impactTime, attackIntent, $"{attacker.name} hits with {action.Name}");
            ...
            timeline.Schedule(endTime, recoveryIntent, $"{attacker.name} recovers");
        }

        public static void ScheduleMove(BattleTimeline timeline, CombatUnit unit, List<Pathfinder.GridPoint> path)
        {
            ...
            for (int i = 1; i < path.Count; i++)
            {
                ...
                timeline.Schedule(accumulatedDelay, moveIntent, $"Move Step {i}");
            ...
            timeline.Schedule(accumulatedDelay, endIntent, "Move End");
        }

        public static void ScheduleDodge(BattleTimeline timeline, CombatUnit unit, float startTime, float duration = 0.5f, float focusCost = 1f)
        {
            ...
            for (int i = 0; i <= steps; i++)
            {
                ...
                timeline.Schedule(startTime + delay, intent, $"{unit.name} Dodges (Window {delay:F2}s)");
            }

            // Schedule window end - reset IsActing state
            var endIntent = new StateChangeIntent(unit, "Idle");
            timeline.Schedule(startTime + duration, endIntent, $"{unit.name} Dodge Window End");
        }

        public static void ScheduleBlock(BattleTimeline timeline, CombatUnit unit, float startTime, float duration = 1.0f, float focusCost = 2f)
        {
            // Interaction Window Implementation:
            ...
            for (int i = 0; i <= steps; i++)
            {
                ...
                timeline.Schedule(startTime + delay, intent, $"{unit.name} Blocks (Window {delay:F2}s)");
            }

            // Schedule window end - reset IsActing state
            var endIntent = new StateChangeIntent(unit, "Idle");
            timeline.Schedule(startTime + duration, endIntent, $"{unit.name} Block Window End");
        }

        public static void ScheduleKnockback(BattleTimeline timeline, CombatUnit unit, GridDirection direction, int distance, float impactSpeed)
        {
            ...
            unit.IsActing = true; // Lock unit during knockback
            ...
            for (int i = 0; i < distance; i++)
            {
                ...
                timeline.Schedule(accumulatedDelay, moveIntent, $"Knockback Step {i}");
            ...
            // 3. End State
            var endIntent = new StateChangeIntent(unit, "Idle");
            timeline.Schedule(accumulatedDelay, endIntent, "Knockback End");
        }
    }
}
